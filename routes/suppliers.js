...
router.route("/add").post(async (req, res) => {
    try {
        ...
        if (!supplier_id || !supplier_name || !contact || credits === undefined) {
            return res.status(400).json({ error: "All fields are required" });
        }

        const newSupplier = new Supplier({
            supplier_id: Number(supplier_id), // Ensure it's a number
            supplier_name,
            contact,
            credits: Number(credits), // Ensure it's a number
        });
        ...
        console.error("Error adding supplier:", err);
        if (err.code === 11000) { // Duplicate key error (supplier_id)
