...
function Workout() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Transform data to match ResultDisplay expectations
      const transformedData = {
        message: response.data.message || '✅ Workout Plan Generated Successfully!',
        workoutPlan: response.data.plan
          ? Object.keys(response.data.plan).map((day, index) => ({
              section: `Day ${index + 1}`,
              rows: [{ day, details: response.data.plan[day].join(', ') }]
            }))
